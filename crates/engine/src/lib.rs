...
#[cfg(test)]
mod tests {
    use super::{derive_assignment_lhs, line_col_at, pattern_required_literals};

    fn lits(text: &str) -> Vec<String> {
        pattern_required_literals(text)
    }
    ...
    fn extracts_code_identifiers_ignoring_inline_regex_strings() {
        ...
        // Nothing from inside the strings leaks in.
    ...
    fn bails_on_raw_regex_alternation_and_quantifiers() {
        // Raw pattern-regex text: identifiers are not guaranteed required.
